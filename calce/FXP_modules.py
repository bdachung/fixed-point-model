...
class MCTransformer(nn.Module):
    def __init__(self, embedding_mode, out_embed_dim, dropout=0, layer_norm_eps=1e-5, num_cycles=8):
...
class FeatureBuilder(nn.Module): #num_labels, num_cycles, in_embed_dim, out_embed_dim, hidden_dim,
    def __init__(self, mode, out_embed_dim, window_size=8, extract_points_len=11, local_features=['voltage','current','resistance'], global_features = ['x_5', 'rest_period', 'capacity']):
        """ Mode: Conv1D_k3mk3, Conv1D_k5k5, LSTM_uni, LSTM_bi, FLatten """

        super(FeatureBuilder, self).__init__()
        self.mode = mode
        if self.mode == 'Conv1D_k3mk3':
            self.conv1 = torch.nn.Conv1d(in_channels=len(local_features), out_channels=16, kernel_size=3, stride=1, padding=0, bias=True)
            ...
            self.in_proj = nn.Linear(32 + len(global_features), out_embed_dim)
        elif self.mode == 'Conv1D_k5k5':
            self.conv1 = torch.nn.Conv1d(in_channels=len(local_features), out_channels=16, kernel_size=5, stride=1, padding=0, bias=True)
            ...
            self.in_proj = nn.Linear(48 + len(global_features), out_embed_dim)
        elif self.mode == 'LSTM_uni':
            self.gru = torch.nn.GRU(input_size=len(local_features), hidden_size=30, num_layers=1, batch_first=True, bidirectional=False)
            # output of LSTM is 30
            self.in_proj = nn.Linear(30 + len(global_features), out_embed_dim)
        elif self.mode == 'LSTM_bi':
            self.gru = torch.nn.GRU(input_size=len(local_features), hidden_size=15, num_layers=1, batch_first=True, bidirectional=True)
            # output of LSTM is 2*15=30
            self.in_proj = nn.Linear(30 + len(global_features), out_embed_dim)
        elif self.mode == 'Flatten':
            self.in_proj = nn.Linear(extract_points_len * len(local_features) + len(global_features), out_embed_dim)
        

    def forward(self, channels_features, global_features): # batch, num_cycles, num_points, sequence_feature_count=3
        """input shape: (batch, cycles, num_points, local_features) & (batch, cycles, global_features)"""                                                    # batch, num_cycles, global_feature_count = 6
...
class Regressor(nn.Module):
    def __init__(self, model_mode, feature_mode, use_AR, num_labels, out_embed_dim, hidden_dim, dropout=0., window_size=8, extract_points_len=11, local_features=['voltage','current','resistance'], global_features = ['x_5', 'rest_period', 'capacity']):
        super(Regressor, self).__init__()
        self.mode = model_mode 
        self.use_AR = use_AR
        if model_mode == 'bert-newposcode' or model_mode == 'bert-trainposcode':
            self.featurebuild = FeatureBuilder(feature_mode, out_embed_dim - 1, window_size=window_size, extract_points_len=extract_points_len, local_features=local_features, global_features=global_features)
        else:
            self.featurebuild = FeatureBuilder(feature_mode, out_embed_dim, window_size=window_size, extract_points_len=extract_points_len, local_features=local_features, global_features=global_features)

        if self.mode == 'bert':
            self.simplebert = MCTransformer('Original', out_embed_dim, dropout, num_cycles=window_size) 
        elif self.mode == 'bert-newposcode':
            self.simplebert = MCTransformer('ConstSeries', out_embed_dim, dropout, num_cycles=window_size) 
        elif self.mode == 'bert-trainposcode':
            self.simplebert = MCTransformer('Trainable', out_embed_dim, dropout, num_cycles=window_size) 
        elif self.mode == 'lstm':
        ...
        self.highway = nn.Linear(window_size, 1, bias=True)
